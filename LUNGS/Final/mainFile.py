...
def load_data(path, split=0.2):
    ...
    for img_fl in tqdm(img_files):
        if(img_fl.split('.')[-1]=='tif'):
            images_list.append('../2d_images/{}'.format(img_fl))
            masks_list.append('../2d_masks/{}'.format(img_fl))
...
def evaluateModel(model, X_test, Y_test, batchSize):
    ...
    fp = open('models/log_drrmsan_lungs.txt','a')
    fp.write(str(jacard)+'\n')
    fp.close()

    fp = open('models/best_drrmsan_lungs.txt','r')
    best = fp.read()
    fp.close()
...
def f(x):
    ...
    # save to json:
    hist_json_file = 'history_lungs_drrmsan.json'
    ...
    # or save to csv:
    hist_csv_file = 'history_lungs_drrmsan.csv'
    ...
    model.save_weights("lungs_drrmsan_150e.h5")
    ...
    fp = open('models/log_drrmsan_lungs.txt','w')
    fp.close()
    fp = open('models/best_drrmsan_lungs.txt','w')
    fp.write('-1.0')
    fp.close()
...
constraints = [{'name': 'constr_1', 'constraint':  '0.9998 - x[:,0] - x[:,1] - x[:,2] - x[:,3]'},
               {'name': 'constr_1', 'constraint': '-0.9999 + x[:,0] + x[:,1] + x[:,2] + x[:,3]'}]
