...
for train_index, test_index in kf.split(X):
    ...
    def saveModel(model):
        ...
        fp = open('models/modelP_attnR2Unet_chest.json','w')
        fp.write(model_json)
        model.save_weights('models/modelW_attnR2Unet_chest.h5')
    ...
    def evaluateModel(model, X_test, Y_test, batchSize):
        ...
        fp = open('models/log_attnR2Unet_chest.txt','a')
        fp.write(str(jacard)+'\n')
        fp.close()

        fp = open('models/best_attnR2Unet_chest.txt','r')
        best = fp.read()
        fp.close()
        ...
        if(jacard>float(best)):
            ...
            print('***********************************************')
            fp = open('models/best_attnR2Unet_chest.txt','w')
            fp.write(str(jacard))
            fp.close()
    ...
    def trainStep(model, X_train, Y_train, X_test, Y_test, epochs, batchSize):
        ...
        # save to json:
        hist_json_file = 'history_attnR2Unet_chest_fold_{}.json'.format(fold_no)
        ...
        # or save to csv:
        hist_csv_file = 'history_attnR2Unet_chest_fold_{}.csv'.format(fold_no)
    ...
    fp = open('models/log_attnR2Unet_chest.txt','w')
    fp.close()
    fp = open('models/best_attnR2Unet_chest.txt','w')
    fp.write('-1.0')
    fp.close()
