...
for train_index, test_index in kf.split(X):
    ...
    def saveModel(model):
        ...
        fp = open('models/modelP_orunet_kvasir.json','w')
        fp.write(model_json)
        model.save_weights('models/modelW_orunet_kvasir.h5')
    ...
    def evaluateModel(model, X_test, Y_test, batchSize):
        ...
        fp = open('models/log_orunet_kvasir.txt','a')
        fp.write(str(jacard)+'\n')
        fp.close()

        fp = open('models/best_orunet_kvasir.txt','r')
        best = fp.read()
        fp.close()
        ...
        if(jacard>float(best)):
            ...
            print('***********************************************')
            fp = open('models/best_orunet_kvasir.txt','w')
            fp.write(str(jacard))
            fp.close()
    ...
    def trainStep(model, X_train, Y_train, X_test, Y_test, epochs, batchSize):
        ...
        # save to json:
        hist_json_file = 'history_orunet_kvasir_fold_{}.json'.format(fold_no)
        ...
        # or save to csv:
        hist_csv_file = 'history_orunet_kvasir_fold_{}.csv'.format(fold_no)
    ...
    fp = open('models/log_orunet_kvasir.txt','w')
    fp.close()
    fp = open('models/best_orunet_kvasir.txt','w')
    fp.write('-1.0')
    fp.close()
