...
def evaluateModel(model, X_test, Y_test, batchSize):
    ...
    fp = open('models/log_attn_1_kvasir.txt','a')
    fp.write(str(jacard)+'\n')
    fp.close()

    fp = open('models/best_attn_1_kvasir.txt','r')
    best = fp.read()
    fp.close()
    ...
    if(jacard>float(best)):
        ...
        print('***********************************************')
        fp = open('models/best_attn_1_kvasir.txt','w')
        fp.write(str(jacard))
        fp.close()
...
def trainStep(model, X_train, Y_train, X_test, Y_test, epochs, batchSize):
    ...
    # save to json:
    hist_json_file = 'history_attn_1_kvasir.json'
    ...
    # or save to csv:
    hist_csv_file = 'history_attn_1_kvasir.csv'
...
fp = open('models/log_attn_1_kvasir.txt','w')
fp.close()
fp = open('models/best_attn_1_kvasir.txt','w')
fp.write('-1.0')
fp.close()
